...
def get_mock_context():
    """Helper to create a mock context for template tag tests."""
...
class TestTemplateButtonAmountValidation:
    ...
    def test_total_at_minimum_passes(self):
        ...
        result = athm_button(get_mock_context(), config)
...
class TestTemplateButtonConfigOutput:
    ...
    def test_output_includes_csrf_token_from_context(self):
        """Test output includes csrf_token from request context."""
        config = get_valid_config()
        context = {"csrf_token": "my-csrf-token"}
        result = athm_button(context, config)
        assert result["csrf_token"] == "my-csrf-token"

    def test_output_csrf_token_empty_when_not_in_context(self):
        """Test output csrf_token is empty string when not in context."""
        config = get_valid_config()
